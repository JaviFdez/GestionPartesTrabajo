...
class AngularApp {
    ...
    /** Servicio para gestionar logs en angular */
    ...
    private _previusLocation: string;

    /** Default constructor */
    constructor() {

        // create angular main module
        let modules: string[] = ["ngSanitize", "ngAnimate", "ngRoute", "LocalStorageModule", "ngCookies", "pascalprecht.translate", //"angular-bootstrap",
        ...
        requirejs(["app/services/httpInterceptor.service",
            ...
            "app/components/mainFooter.component",
            "app/components/mainHeader.component"], () => {
                this._angularConfig();
                this._angularRun();
                // start angular app
    ...
    public get PreviusLocation(): string {
        return this._previusLocation
    }
    ...
    private _angularRun(): void {
        ...
        let fn: any[] =
            ["$rootScope", "$location", "$translate", "$log", "$injector",
                ($rootScope: ng.IRootScopeService, $location: ng.ILocationService,
                $translate: ng.translate.ITranslateService, $log: ng.ILogService,
                $injector: ng.auto.IInjectorService) => {
                    ...
                    this._authService = $injector.get("authService") as IAuthService;
                    this._usersService = $injector.get("usersService") as IUsersService;

                    // verificar el rol de usuario al acceder a la ruta
                    $rootScope.$on("$routeChangeStart", (_event: ng.IAngularEvent, _next: IRouteDefinition, _current: IRouteDefinition) => {
                        this._previusLocation = (_current && (_current as any).originalPath) ? (_current as any).originalPath : "";
 
                        if (_next && _next.requireUserRole && _next.requireUserRole.length > 0) {
 
                            // Verificar el/los roles permitidos para la pagina
                            this._usersService.getCurrentUser().then((user: IUserModel) => {
 
                                if (!user || _next.requireUserRole.indexOf(user.Rol) === -1) {
                                    $rootScope.$evalAsync(() => {
                                        $location.path("login");
                                    });
                                }
